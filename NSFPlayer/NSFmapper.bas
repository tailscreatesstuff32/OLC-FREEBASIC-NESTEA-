...
Type NSFMapper extends object
banked As boolean
origbanks As uint8_t Ptr

loadaddr As uint16_t
banks(8) As uint8_t 'Ptr = New uint8_t(7)
...
Declare Constructor(bytes_data As TVECTORUINT8T ,loadadr As uint16_t,banked As boolean,banks As uint8_t ptr)




Declare function read_NSF(As uint16_t) As uint8_t 
...
Constructor NSFMapper(bytes_data as TVECTORUINT8T, loadadr As uint16_t,banked As boolean,banks As uint8_t Ptr)
 
banked = banked
this.origbanks = @banks
this.loadaddr = loadadr
this.data1.resize(bytes_data.size()):this.data1 = bytes_data 
...
Next
	...
	If this.banked Then
		...
		for i as uint16 = this.loadaddr to ubound(romdata)-1
			if &H80 + (i - this.loadAddr) >=  this.data1.size()  then
		
		   exit for
		end if
		romdata(i) = this.data1[&H80 +(i-this.loadAddr)]
