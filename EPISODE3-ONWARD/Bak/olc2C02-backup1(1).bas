...
Function GetColourFromPaletteRam( pal1 As uint8_t,  pixel As uint8_t) As ULong
...
sub GetPatternTable OverLoad( i As uint8_t,  Pal1 As uint8_t)' As bool' Any Ptr
		...
		for nTileX As uint16_t = 0 To  16-1
			...
			for  row As uint16_t = 0 to 8 -1 
				...
				pixel = (tile_lsb and  &H01) + (tile_msb and  &H01) 
					...
					PSet((nTileX * 8 + (7 - col)),(nTileY * 8 + row)),GetColourFromPaletteRam(pal1, pixel)
...
If (cart_mirror =  VERTICAL) Then
	...
	elseif (cart_mirror = HORIZONTAL) then
	'	
	'		// Horizontal
...
End If
	...
	If (mask.render_background) Then
		...
		'// at the required location.
		 p0_pixel = (bg_shifter_pattern_lo And bit_mux) > 0 
		 p1_pixel = (bg_shifter_pattern_hi and bit_mux) > 0 

	'	// Combine to form pixel index
		bg_pixel = (p1_pixel Shl 1)or p0_pixel 

		'// Get palette
		'bg_pal0 = (bg_shifter_attrib_lo And bit_mux) > 0 
		'bg_pal1 = (bg_shifter_attrib_hi And bit_mux) > 0 
...
'	sprScreen.SetPixel(cycle - 1, scanline, GetColourFromPaletteRam(bg_palette, bg_pixel));
   ...
   'PSet(nesscrn,cycle-1,scanline,GetColourFromPaletteRam(bg_palette, bg_pixel))
	...
	'PSet nesscrn,((cycle-1),(scanline)),GetColourFromPaletteRam(bg_palette, bg_pixel)
 	  ...
 	  Line nesscrn,((cycle-1)*2,  ((scanline)*2))-STEP(2 OR 1, 2 OR 1),GetColourFromPaletteRam(0, bg_pixel) , BF 
